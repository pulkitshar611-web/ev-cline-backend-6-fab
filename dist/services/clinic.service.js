...
export const getClinicContext = async (clinicId) => {
...
export const addStaff = async (clinicId, data) => {
    try {
        ...
        if (primaryRoleRaw === 'LABORATORY')
            primaryRoleRaw = 'LAB';
        if (primaryRoleRaw === 'PHARMACIST')
            primaryRoleRaw = 'PHARMACY';
...
export const deleteClinicStaff = async (clinicId, staffId, userRole) => {
    const staff = await prisma.clinicstaff.findUnique({
        where: { id: staffId }
    });
    if (!staff)
...
export const getDoctorAvailability = async (clinicId, doctorId) => {
...
export const createClinicService = async (clinicId, data) => {
...
export const updateClinicService = async (clinicId, serviceId, data) => {
    const { name, description, price, type, isActive } = data;
